<h1 align="center">Hi 👋, I'm Ashvek Parab</h1>
<h3 align="center">Computer Engineering Student | Full-Stack Developer | AI & IoT Enthusiast</h3>

<p align="center">
  Building real-world software through projects, hackathons, and continuous learning.
</p>

---

## 👨‍💻 About Me

- 🎓 Computer Engineering Student at **Agnel Institute of Technology and Design, Goa**
- 📊 **9.8+ CGPA**
- 🏆 Winner of **3 Hackathons**
- 💡 Interested in **Software Development, Artificial Intelligence, IoT, and Computer Vision**
- 🌱 Always learning new technologies and exploring better ways to solve real-world problems
- 🤝 Enjoy collaborating on innovative projects and hackathons

---

## 🚀 Featured Projects

### 🌊 FloodVision
AI-powered flood monitoring and emergency response platform integrating weather intelligence, IoT sensors, flood prediction, and Google Maps for safer navigation.

### ⚡ SEROS
Smart Energy & Resource Optimization System using YOLOv8, Django, Arduino, and Machine Learning for intelligent energy management.

### ❤️ MindGuard
AI + IoT healthcare platform featuring disease detection, AI chatbot, voice stress analysis, and ESP32-based fall detection.

### 🎓 CareerPath
AI-powered career guidance platform developed for Smart India Hackathon (SIH) 2025 with personalized recommendations, resume builder, and learning resources.

### 🚚 Construction Logistics Management System
Developed a full-stack logistics management platform for **Sheshank Transport**, enabling customers to order construction materials, track deliveries, manage inventory, calculate distance-based pricing, and streamline business operations through an admin dashboard.

---

<h2>TECHNICAL SKILLS</h2>
<div class="skills-list">
    <p><strong>Languages:</strong> C, C++, Python, Java, JavaScript</p>
    <p><strong>Technologies:</strong> HTML, CSS, React, React Native, Django, Node.js, Flutter</p>
    <p><strong>Domains:</strong> Artificial Intelligence, Machine Learning, Computer Vision, IoT, Full-Stack Development</p>
    <p><strong>Tools:</strong> Git, GitHub, Figma, XAMPP, Arduino</p>
    <p><strong>Core CS:</strong> DSA, OOP, DBMS, Software Engineering</p>
</div>

---

## 🎯 Current Focus

- 🤖 Artificial Intelligence & Machine Learning
- 🌐 Full-Stack Development
- 📱 Mobile Application Development
- 🧩 Data Structures & Algorithms
- 🚀 Building impactful software projects

---

## 📈 GitHub Stats

<p align="center">
  <!-- Add GitHub stats cards here -->
</p>

---

## 📫 Connect With Me

- 📧 **Email:** ashvekparab2006@gmail.com
- 💼 **LinkedIn:** https://www.linkedin.com/in/ashvek-parab
- 💻 **GitHub:** https://github.com/AshvekParab7

---

<p align="center">
  <i>"Learning, building, and improving—one project at a time."</i>
</p>
